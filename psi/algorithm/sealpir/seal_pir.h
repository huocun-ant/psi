...
class SealPirServer : public SealPir, public psi::pir::IndexPirServer {
 public:
  ...
  yacl::Buffer GenerateIndexReply(
...
class SealPirClient : public SealPir, public psi::pir::IndexPirClient {
 public:
  ...
  std::pair<yacl::Buffer, uint64_t> GenerateIndexQuery(
      uint64_t raw_idx) const override;

  seal::GaloisKeys GenerateGaloisKeys() const;

  seal::Plaintext DecodeReply(PirReply &reply) const;
  std::vector<uint8_t> DecodeReply(PirReply &reply, uint64_t offset) const;
 ...
 private:
  ...
  std::vector<uint8_t> ExtractBytes(seal::Plaintext pt, uint64_t offset) const;
};
}  // namespace psi::sealpir
