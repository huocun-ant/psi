...
void SpiralServer::SetDatabase(
  ...
  YACL_ENFORCE_EQ(raw_database[0].size(), database_info_.byte_size_per_row_);

  size_t max_bit_length =
      params_.N() * params_.N() * params_.PolyLen() * params_.PtModulusBitLen();
  YACL_ENFORCE(database_info_.byte_size_per_row_ * 8 <= max_bit_length,
               "The length of one row of raw data exceeds the maximum length "
               "that can be holded");
  // here, we only consider one row of raw data can be holded by one plaintext
  // in SpiralPIR
  size_t element_byte_len = database_info_.byte_size_per_row_;
  ...
  reoriented_db_ = std::move(reorient_db);
...
std::vector<PolyMatrixRaw> SpiralServer::ProcessQuery(
  ...
  for (size_t ins = 0; ins < params_.Instances(); ++ins) {
    ...
    for (size_t trial = 0; trial < n_power; ++trial) {
      size_t idx = (ins * n_power + trial) * db_slice_sz;
      ...
      for (size_t i = 0; i < num_per; ++i) {
        for (size_t c = 0; c < pt_cols; ++c) {
          ...
          for (size_t jm = 0; jm < dim0 * pt_rows; ++jm) {
            uint64_t b = reoriented_db_[cur_db_idx + idx_b_base];
            idx_b_base += 1;
