...
class SpiralServer {
 public:
  ...
  explicit SpiralServer(Params params, std::vector<uint64_t> reoriented_db)
      : params_(std::move(params)), reoriented_db_(std::move(reoriented_db)) {
  ...
  explicit SpiralServer(Params params, std::vector<uint64_t> reoriented_db,
      ...
      : params_(std::move(params)),
        reoriented_db_(std::move(reoriented_db)),
  ...
  void SetDatabase(const std::vector<std::vector<uint8_t>>& raw_database);
 ...
 protected:
  ...
  void MultiplyRegByDatabase(std::vector<PolyMatrixNtt>& out,
                             ...
                             size_t num_per, size_t cur_db_idx = 0) const;
 ...
 private:
  ...
  std::vector<uint64_t> reoriented_db_;
