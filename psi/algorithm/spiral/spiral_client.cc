...
std::vector<uint8_t> SpiralClient::DecodeResponse(
  ...
  // first we obtain the inter decode result
  yacl::ElapsedTimer timer;

  auto decode_result = DecodeResponseInternal(response);
  ...
  // todo: consider more general impl for large value length
  size_t element_byte_len = database_info_.byte_size_per_row_;
  std::vector<uint8_t> result(database_info_.byte_size_per_row_);
  // just copy
  std::memcpy(result.data(), bytes.data() + offset * element_byte_len,
